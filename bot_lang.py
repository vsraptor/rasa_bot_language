...
#------------ STORY --------------------------------------------
	...
	def story_utter(self, (utter,)):
		...
		self.add2domain('actions',utter_name)
...
#------------ INTENT --------------------------------------------
	...
	# repeated intents are merged
	def intent(self, items):
		rv = "%s" % items[2]
		if items[1] in self.data.intents : #merge intent
			self.data.intents[items[1]] += rv
		else : #new intent
			rv = ("## intent:%s\n" % items[1]) + rv 
			self.data.intents[items[1]] = rv
			#... add to domain
			self.add2domain('intents', items[1])
		return rv
...
#------------ UTTER --------------------------------------------
	...
	def utter(self, items):
		...
		self.add2domain('actions', 'utter_' + items[1])
...
#------------ DOMAIN --------------------------------------------
	...
	def domain(self, items):
		for i in items[1:] :
			if not isinstance(i, (Token,Tree)) :
				self.add2domain(items[0],i)
	#			return items

	def domain_kws(self, items): return items[0]

	def slots(self, items):
 		for i in items : self.add2domain('slots',i)

	def slot(self, items):
		return "%s : %s" % (items[0], items[1].render())

	def add2domain(self, key, value):
		dkey = self.data.domain[key]
		if value not in dkey : dkey.append(value) #only if doesnt exists

#------------BASIC --------------------------------------------

	def join(self, items, prefix='', suffix='', quote=False, indent='', ident_first=False, eos=''):
